...
perform_cleanup() {
    ...
    if [ -d "$GKI_ROOT/KernelSU" ]; then
        rm -rf "$GKI_ROOT/KernelSU" && echo "[-] KernelSU-Next directory deleted."
    fi
}
...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU" || git clone https://github.com/Taufik-Daulay/KernelSU && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout susfs-155 && echo "[-] Switched to next-susfs-dev branch."
    fi
